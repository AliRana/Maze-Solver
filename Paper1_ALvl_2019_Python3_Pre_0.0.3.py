...
def GetInstruction():
  ...
  return Instruction #You input a command to the program and it converts it to lower, before returning that value

def ExtractCommand(Instruction): #The Instruction returned from GetInstruction is used as a parameter(passing a parameter)
  Command = ""
  if " " not in Instruction:
    return Instruction, Instruction
  while len(Instruction) > 0 and Instruction[0] != " ":
    Command += Instruction[0] #Adds the value from Instruction[0] and then changes Instruction value
    Instruction = Instruction[1:] #Makes Instruction equal to everything to the right of the position [1:]
  while len(Instruction) > 0 and Instruction[0] == " ":
    Instruction = Instruction[1:] #This section of code is used to remove the speech marks fromthe begining of the variable
...
def DisplayDoorStatus(Status): #Checks the parameter for the subroutine for a certain value, then print output to screen
  if Status == "open":
...
def DisplayContentsOfContainerItem(Items, ContainerID):
  ...
  ContainsItem = False  #Boolean Variable
  for Thing in Items:	#Passing Items to Things     #Check the variable list to see if something is in there
    if Thing.Location == ContainerID: #2D Array or a list or lists(Data Structures) / Dictionary (The name Thing is the dictionary and the .location is using a keyword to find a value in a specific location.
      if ContainsItem:
        print(", ", end = "")
      ContainsItem = True
      print(Thing.Name, end = "") #The same concept as before but using the location/keyword Name
  if ContainsItem: #Incomplete if statement
    print(".")
  else:
    print("nothing.")
...
def Examine(Items, Characters, ItemToExamine, CurrentLocation):
  Count = 0
  if ItemToExamine == "inventory": #Option of whether your examining your inventory or an object
    DisplayInventory(Items)  #Calls Inventory subroutine using the list items
  else:
    IndexOfItem = GetIndexOfItem(ItemToExamine, -1, Items)#Finds index of item in the item list
    if IndexOfItem != -1:
      if Items[IndexOfItem].Location == INVENTORY or Items[IndexOfItem].Location == CurrentLocation: #This checks whether the item is something that can be examine, something in the inventory or something in the current location
        print(Items[IndexOfItem].Description) #This gets the Description from the Items list at the position index [IndexOfItems]
        if "door" in Items[IndexOfItem].Name: # If the name of the Item at the index is door then call Subroutine with the parameters passed to it
          DisplayDoorStatus(Items[IndexOfItem].Status)
        if "container" in Items[IndexOfItem].Status: # If the status of the item at the index is a container then call DisplayContentOfContainerItem and pass the subroutine parameters
          DisplayContentsOfContainerItem(Items, Items[IndexOfItem].ID)
        return #Missing returing variable/object
    while Count < len(Characters): #Checks if Characters holds any values
      if Characters[Count].Name == ItemToExamine and Characters[Count].CurrentLocation == CurrentLocation:
        print(Characters[Count].Description)#If the item is equal to a name in the ItemToExamine list and the index of the Character currentlocation is the same as the current location then return
        return
      Count += 1
    print("You cannot find " + ItemToExamine + " to look at.") #Syntax Errors

def GetPositionOfCommand(CommandList, Command):
  Position = Count = 0 #Diffrent from 
...
def Teleport(You, Location):
  You.CurrentLocation = int(Location) #Uses the class YOU to change the location to the inputted location
  return You, True
...
def PlayGame(Characters, Items, Places):
  ...
  while not StopGame:
    ...
    if Command == "get":
      ...
      StopGame = True
    elif Command == "help": #If you have used the word help it calls this code
      CommandList = ["get", "use", "go", "read", "examine", "open", "close", "move", "say", "playdice"]
      print("Commands:") 
      for i in CommandList:
        print(i,end="\n") #Prints all the items in the ComandList
    elif Command == "teleport":
      Characters[0], Moved = Teleport(Characters[0],Instruction) #This calls the teleport function and passes it the two parameters
    else:
...
##Prevent guard from taking dice
##def TakeRandomItemFromPlayer(Items, OtherCharacterID):
##  ListOfIndicesOfItemsInInventory = []
##  Count = 0
##  while Count < len(Items):
